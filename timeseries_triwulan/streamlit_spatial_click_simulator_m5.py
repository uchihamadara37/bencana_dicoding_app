...
CALIBRATION_PATH = ARTIFACT_DIR / "calibration_params_m5.json"
...
@st.cache_data
def load_calibration_params(path: Path):
    if not path.exists():
        return None
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def calibrate_total_prediction(
    total_raw: float, calibration_params: dict | None
) -> dict:
    if calibration_params is None:
        return {
            "is_calibrated": False,
            "method": "none",
            "total_calibrated": float(total_raw),
            "alpha": None,
        }

    alpha = float(calibration_params.get("ratio_scale_alpha", 1.0))
    total_calibrated = float(total_raw * alpha)
    return {
        "is_calibrated": True,
        "method": "ratio_scale_alpha",
...
calibration_params = load_calibration_params(CALIBRATION_PATH)
...
if calibration_params is not None:
    alpha_for_table = float(calibration_params.get("ratio_scale_alpha", 1.0))
    monthly_eval_df["pred_expected_total_calibrated"] = (
        monthly_eval_df["pred_expected_total"] * alpha_for_table
    )
    monthly_eval_df["abs_error_calibrated"] = (
        monthly_eval_df["actual_total"]
        - monthly_eval_df["pred_expected_total_calibrated"]
    ).abs()
...
with st.sidebar:
    ...
    use_calibration = st.checkbox(
        "Gunakan kalibrasi total bulanan (ratio)",
        value=calibration_params is not None,
        disabled=calibration_params is None,
    )

    if calibration_params is None:
        st.caption(
            "Kalibrasi belum ditemukan. Jalankan notebook kalibrasi untuk mengaktifkan opsi ini."
        )
    else:
        st.caption(
            f"Kalibrasi aktif tersedia: alpha={float(calibration_params.get('ratio_scale_alpha', 1.0)):.4f}"
        )
...
if run_btn:
    ...
    total_raw = float(total_result["total_expected_count"])
    calib_result = calibrate_total_prediction(total_raw, calibration_params)
    total_for_display = (
        float(calib_result["total_calibrated"])
        if use_calibration and calib_result["is_calibrated"]
        else total_raw
    )
    mode_label = (
        "calibrated" if (use_calibration and calib_result["is_calibrated"]) else "raw"
    )
    ...
    st.markdown(
        ...
        <div style="border:2px solid #0f766e;border-radius:14px;padding:18px;background:#ecfeff;">
            <div style="font-size:16px;font-weight:600;color:#115e59;">Estimasi total gempa M&gt;=5 untuk bulan {total_result['target_month']}</div>
            <div style="font-size:56px;font-weight:800;line-height:1.1;color:#0f766e;">{total_for_display:.1f}</div>
    ...
    st.caption(
        f"Mode: {mode_label}. Raw={total_raw:.1f}"
        + (
            f", Calibrated={float(calib_result['total_calibrated']):.1f} (alpha={float(calib_result['alpha']):.4f})"
